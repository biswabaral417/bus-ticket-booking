...
const BusSeat = ({ seatNumber, finalDate, seatClick, selBus,isSelected,isPending }) => {
    const findAppMargin=()=>{
        let margin="0px"
        if (seatNumber==="A1") {
            return "80px"
        ...
        if(seatNumber.substring(0,1)==="A"){
            if(((selBus.busRows*2)-1)===Number(seatNumber.substring(1))){
                return "0px"
            }
            else{
                margin=seatNumber.substring(0,1)==="A"?seatNumber.substring(1)%2!==0?"40px":"":""
                return margin
            }
        }
    }
    ...
    return (
        ...
        >
            ...
            <svg width="800px"
                className={`${isPending?"pending":""} ${isSelected?"selected":""}`}
