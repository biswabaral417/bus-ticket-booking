...
export default function BusSeat({seatNumber}) {
  const {  finalDate,
    bus,
    selectedSeats,
    userSelectedSeat,
    ...
    bookedSeats,
    setBookedSeats,
    socket}=useContext(BookingsContext)
  ...
  const findAppMargin = () => {
    ...
    if (seatNumber.substring(0, 1) === "A") {
      if (bus.busRows * 2 - 1 === Number(seatNumber.substring(1))) {
        return "0px";
      } else {
        margin =
          seatNumber.substring(0, 1) === "A"
            ? seatNumber.substring(1) % 2 !== 0
              ? "50px"
              : ""
            : "";
        return margin;
  ...
  const seatClick = () => {
    const isNotAvailable =
      bus.bookings.seatsBooked.includes(seatNumber) ||
      (selectedSeats.includes(seatNumber) &&
        !userSelectedSeat.includes(seatNumber));
    if (isNotAvailable) {
      return;
    }
    ...
    socket.emit("seat-clicked", seatNumber, finalDate, bus);
    if (userSelectedSeat.includes(seatNumber)) {
      setSelectedSeat((prevSelectedSeats) =>
        prevSelectedSeats.filter((seat) => seat !== seatNumber),
      );
      setUserSelectedSeat((prevSelectedSeats) =>
        prevSelectedSeats.filter((seat) => seat !== seatNumber),
      );
      return;
    }
    setUserSelectedSeat((prevUserSelectedSeats) => [
      ...prevUserSelectedSeats,
      seatNumber,
    ]);
  };
  
  useEffect(() => {
    socket.on("busSeatSelected", (seatNo) => {
      if (seatNo === seatNumber) {
        if (selectedSeats.includes(seatNo)) {
          ...
          setSelectedSeat((prevSelectedSeat) => [...prevSelectedSeat, seatNo]);
        }
      }
    });
  ...
  return (
    <button
      onClick={(e) => seatClick(e, { seatNumber, finalDate, bus })}
      style={{ marginRight: findAppMargin() }}
      className={`busSeat c_pointer`}
    >
      <p className="t_a_c">{seatNumber}</p>
      <svg
        width="800px"
        className={`${selectedSeats.includes(seatNumber) ? "pending" : ""} ${userSelectedSeat.includes(seatNumber) ? "selected" : ""} ${    bookedSeats.includes(seatNumber) ? "booked" : ""}`}
        height="800px"
        viewBox="0 0 24 24"
        fill=""
