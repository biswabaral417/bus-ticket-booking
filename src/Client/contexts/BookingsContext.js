...
const BookingsContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on("busSeatsBooked", selSeatsArr => {
      setBookedSeats((prevBooked) => prevBooked.concat(selSeatsArr));
      setSelectedSeat((prevSel) => prevSel.filter((item) => Array.isArray(selSeatsArr) && !selSeatsArr.includes(item)))
    })
