...
const busPending = async ({seatNumber, finalDate, selBus, userToken}) => {
  ...
  console.log("selBus.busNumber:"+selBus.busNumber)
  ...
  try {
    ...
    if (!bookingDate) {
      const newBooking = new Bookings({
        date: finalDate,
        buses: [{ BusNumber: selectedBus.busNumber, seatsSelected: [{userToken:userToken,seatNumber:seatNumber}],seatsBooked:[],userSelected:[] }]
      });
      await newBooking.save();
    
      return seatNumber
    }
    const busPending = bookingDate.buses.find(bus => bus.BusNumber === selectedBus.busNumber);
    if (!busPending) {
      bookingDate.buses = [{ busNumber: selectedBus.busNumber, seatsSelected: [{userToken:userToken,seatNumber:seatNumber}] }]
    ...
    if (busPending.seatsSelected.some(item => item.seatNumber === seatNumber && item.userToken === userToken)) {
      busPending.seatsSelected = busPending.seatsSelected.filter(item => !(item.seatNumber === seatNumber && item.userToken === userToken));
      await bookingDate.save();
    } else {
      // Add the seat to seatsSelected
      busPending.seatsSelected.push({ userToken: userToken, seatNumber: seatNumber });
      await bookingDate.save();
    }
    return seatNumber
